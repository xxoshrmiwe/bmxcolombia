...
const RANKING_QUERY = `*[_type == "piloto" && activo == true] | order(puntos desc) { _id, nombre, slug, foto, numero, departamento, categoria, puntos, victorias, podios, equipo }`
export default async function RankingPage() {
  const pilotos = await client.fetch(RANKING_QUERY)
  return (
        ...
        {pilotos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/5">
            {pilotos.map((piloto: any, i: number) => (
              <div key={piloto._id} className={`bg-[#0D0D0D] p-6 relative border-t-4 hover:-translate-y-1 transition-transform ${i===0?'border-yellow-400':i===1?'border-gray-400':i===2?'border-amber-600':'border-transparent hover:border-red-600'}`}>
