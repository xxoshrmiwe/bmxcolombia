...
import { EVENTOS_PROXIMOS_QUERY } from '@/sanity/queries'
export const revalidate = 300
const badgeColor: Record<string, string> = {
  Nacional: 'bg-red-600 text-white',
  Regional: 'border border-yellow-400 text-yellow-400',
  'Copa UCI': 'bg-yellow-400 text-black',
  Internacional: 'bg-blue-600 text-white',
  Liga: 'border border-gray-400 text-gray-400',
}
export default async function CalendarioPage() {
  const hoy = new Date().toISOString().split('T')[0]
  const eventos = await client.fetch(EVENTOS_PROXIMOS_QUERY, { hoy })
  return (
              ...
              return (
                <div key={evento._id} className="flex items-center gap-8 py-6 hover:pl-4 transition-all group">
                  ...
                  <div className="flex gap-2 items-center">
                    {evento.inscripcionesAbiertas && evento.urlInscripcion && (
                      <a href={evento.urlInscripcion} target="_blank" className="text-xs font-mono px-3 py-1 bg-green-600 text-white uppercase tracking-wider hover:bg-green-500">Inscribirse</a>
                    )}
